...
网页由交易日历驱动：可以进入尚未生成的历史交易日并补跑，周末和交易所休市日不可运行。点击生成后会立即转为独立后台进程，页面无需等待，仍可浏览其他日期；顶部会持续显示运行状态，且同一时间只允许一个网页扫描任务。页面使用“市场全景 / 我的策略 / 人工复盘”三个区域整理上证环境、涨停结构、行业分布、正式候选和人工判断。只有完整全市场收盘快照通过审计时才标记“全市场涨停 / 完整”；历史重建会明确提示免费数据源覆盖限制。
