...
这是一个个人盘后复盘工具：每天收盘后，寻找“中大市值、近20个交易日多次但不连续涨停、处于250日相对低位、两次涨停间有回落”的股票。它不提供实时交易、K线、AI分析、买卖建议或自动交易；个股当前资讯仅在当天为交易日且已到盘后发布时点时按需查询。
...
网页由交易日历驱动：可以进入尚未生成的历史交易日并补跑，周末和交易所休市日不可运行。点击生成后会立即转为独立后台进程，页面无需等待，仍可浏览其他日期；顶部会持续显示运行状态，且同一时间只允许一个网页扫描任务。后台进程在 macOS/Linux 和 Windows 上分别使用安全的平台启动与探活方式。页面使用“市场全景 / 我的策略 / 人工复盘”三个区域整理上证环境、涨停结构、行业分布、正式候选和人工判断。只有完整全市场收盘快照通过审计时才标记“全市场涨停 / 完整”；历史重建会明确提示免费数据源覆盖限制。

每只已识别涨停股票可独立标记“重点、一般、不要”，保存原因和备注。人工判断按交易日和股票代码单独保存，不属于行情扫描快照，重新抓取和历史补跑不会覆盖；系统还会保留 `LIVE` / `HINDSIGHT` 录入时点标记，供以后统计区分当时判断与事后回看。点击“查看详情”会按需加载并缓存截至该复盘日的腾讯前复权历史画像，以及东方财富最近一期财务摘要。免费资讯接口不能保证历史覆盖，因此历史日期、周末查看上一个交易日以及交易日上午都不展示当前资讯，彻底避免未来信息泄漏。
...
每次运行都有独立 run 记录，只有 SUCCESS/WARNING 结果才更新该交易日的 published snapshot。后台更新失败时旧结果继续显示；服务重启会把无法继续的遗留任务标记为 `INTERRUPTED`。数据库默认固定为项目根目录下的 `data/screener.db`，从其他工作目录启动也会使用同一个文件。股票行业只在缺失时按需缓存查询；资讯不会在扫描时批量请求，点击详情时才按需查询。
